...
function createTinRules(): RuleSnapshot {
  const piecesByCode: Record<string, PieceDefinition> = {};
  for (const code of ['FU', 'GI', 'OU', 'TIN', ...PORTED_APP_SKILL_CODES]) {
    piecesByCode[code] = createSilverLikePiece(code);
  }
  return {
    version: 1,
...
function createSilverLikePiece(pieceCode: string): PieceDefinition {
  return {
    pieceCode,
    canonicalCode: pieceCode,
    char: pieceCode === 'TIN' ? '錫' : pieceCode,
    name: pieceCode,
    skill: '',
    moveVectors: [
      { dx: -1, dy: -1, maxStep: 1 },
